namespace VanillaPsycastsExpanded.Chronopath
{
    ...
    public class GameCondition_RaidPause : GameCondition_TimeSnow
    {
        ...
        {
            if (totalDamageDealt >= 0f && dinfo.Def.ExternalViolenceFor(___pawn) && dinfo.Instigator is {Faction: {IsPlayer: true} playerFaction} &&
                ___pawn.HostileTo(playerFaction))
                foreach (GameCondition_RaidPause condition in ___pawn.MapHeld.gameConditionManager.ActiveConditions.OfType<GameCondition_RaidPause>())
                    condition.End();
        }
    }
